...
public class RunDebugTaskAction extends Action {
	TreeViewer viewer; 
	public RunDebugTaskAction(TreeViewer viewer) {
		this.viewer = viewer;
		setText("Run --debug");
		setToolTipText("Log in debug mode (includes normal stacktrace).");
	...
	public void run() {
		...
		if(obj instanceof TreeObject && !(obj instanceof TreeParent))
			RunCommand.getInstance().run("--debug "+((TreeObject)obj).getRun(), true);
	}
}
