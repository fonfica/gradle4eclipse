...
public class RunQuietTaskAction extends Action {
	TreeViewer viewer; 
	public RunQuietTaskAction(TreeViewer viewer) {
		this.viewer = viewer;
		setText("Run --quiet");
		setToolTipText("Log errors only.");
	...
	public void run() {
		...
		if(obj instanceof TreeObject && !(obj instanceof TreeParent))
			RunCommand.getInstance().run("--quiet "+((TreeObject)obj).getRun(), true);
	}
}
